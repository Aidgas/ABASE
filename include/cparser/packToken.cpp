...
nrex* packToken::asRegEx() const {
    
    switch (base->type) {
        case REGEX:
            return static_cast<Token<nrex*>*> (base)->val;
        default:
            if (!(base->type & NUM)) {
                throw bad_cast(
                        "The Token is not a number!");
...
FILE* packToken::asFILE() const {
    
    switch (base->type) {
        case ISFILE:
            return static_cast<Token<FILE*>*> (base)->val;
        default:
            if (!(base->type & NUM)) {
                throw bad_cast(
                        "The Token is not a number!");
...
struct buf* packToken::asBuffer() const {
    
    switch (base->type) {
        case BUFFER:
            return static_cast<Token<struct buf*>*> (base)->val;
        default:
            if (!(base->type & NUM)) {
                throw bad_cast(
                        "The Token is not a number!");
